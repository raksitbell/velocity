...
@Component({
    ...
    <div class="text-zinc-300 antialiased selection:bg-blue-500/30 font-sans">
        ...
        <div class="mt-8 border-t border-zinc-800 pt-6 max-w-3xl">
            ...
            <div class="space-y-4">
                <div *ngFor="let log of logs" class="flex gap-3 text-sm group">
                ...
                <div *ngIf="logs.length === 0" class="text-zinc-600 text-sm italic py-4">No recent activity</div>
            </div>
        </div>
    ...
    <!-- Create Task Modal -->
    <app-modal [(isOpen)]="isTaskModalOpen" title="New Task">
        <div class="space-y-4">
            <div>
                <label class="block text-xs font-medium text-zinc-400 mb-1.5">Task Title</label>
                <input [(ngModel)]="newTaskTitle" (keydown.enter)="confirmAddTask()" type="text" class="w-full bg-zinc-800 border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:ring-1 focus:ring-blue-500 placeholder:text-zinc-600" placeholder="What needs to be done?" autofocus>
            </div>
             <div class="flex justify-end gap-2 pt-4">
                <button (click)="isTaskModalOpen = false" class="px-3 py-1.5 text-zinc-400 hover:text-zinc-200 text-sm">Cancel</button>
...
export class BoardComponent implements OnInit, OnChanges {
        ...
        if (error) {
            ...
            (tasks || []).forEach((task: any) => {
                ...
                if (col) {
                    ...
                    this.columns[0].tasks.push(task as Task);
                }
            });
        }
    ...
    async confirmAddTask() {
        ...
        this.isTaskModalOpen = false; // Close immediately for better UX
        ...
        if (data) {
            ...
            if (col) col.tasks.push(data as Task);
    ...
    async drop(event: CdkDragDrop<Task[]>) {
        if (event.previousContainer === event.container) {
            ...
            // Update DB
            const destColumn = this.columns.find(c => c.tasks === event.container.data);
